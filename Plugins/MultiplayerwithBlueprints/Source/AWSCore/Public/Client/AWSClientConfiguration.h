 ...
 */
UENUM(BlueprintType)
enum class EAWSCoreUseRequestCompression : uint8 {
    DISABLE,
    ENABLE,
};

USTRUCT(BlueprintType)
struct FAWSCoreRequestCompressionConfig {
    GENERATED_USTRUCT_BODY()

    UPROPERTY(EditAnywhere, BlueprintReadWrite, Category="AWS Core")
    EAWSCoreUseRequestCompression useRequestCompression = EAWSCoreUseRequestCompression::ENABLE;

    UPROPERTY(EditAnywhere, BlueprintReadWrite, Category="AWS Core")
    int requestMinCompressionSizeBytes = 10240;
#if WITH_CORE
public:
    Aws::Client::RequestCompressionConfig toAWS() const {
        Aws::Client::RequestCompressionConfig awsRequestCompressionConfig;

        switch (this->useRequestCompression) {
            case EAWSCoreUseRequestCompression::ENABLE:
                awsRequestCompressionConfig.useRequestCompression = Aws::Client::UseRequestCompression::ENABLE;
                break;
            case EAWSCoreUseRequestCompression::DISABLE:
                awsRequestCompressionConfig.useRequestCompression = Aws::Client::UseRequestCompression::DISABLE;
                break;
        }

        awsRequestCompressionConfig.requestMinCompressionSizeBytes = this->requestMinCompressionSizeBytes;

        return awsRequestCompressionConfig;
    }
#endif
};
...
public:
     ...
     */
    UPROPERTY(EditAnywhere, BlueprintReadWrite, Category="AWS Core")
    int64 tcpKeepAliveIntervalMs = 30000;
    ...
    */
     ...
     */
    UPROPERTY(EditAnywhere, BlueprintReadWrite, Category="AWS Core")
    ETransferLibType httpLibOverride = ETransferLibType::DEFAULT_CLIENT;

    /**
     * Sets the behavior how http stack handles 30x redirect codes.
     */
    UPROPERTY(EditAnywhere, BlueprintReadWrite, Category="AWS Core")
    EFollowRedirectsPolicy followRedirects = EFollowRedirectsPolicy::DEFAULT;
...
//    bool enableEndpointDiscovery = false;
     ...
     */
    UPROPERTY(EditAnywhere, BlueprintReadWrite, Category="AWS Core")
    FAWSCoreRequestCompressionConfig requestCompressionConfig;

    /**
     * Disable all internal IMDS Calls
     */
    UPROPERTY(EditAnywhere, BlueprintReadWrite, Category="AWS Core")
    bool disableIMDS = true;

#if WITH_CORE
public:
    Aws::Client::ClientConfiguration toAWS() const {
        Aws::Client::ClientConfiguration awsClientConfiguration(this->profileName.IsEmpty() ? "" : TCHAR_TO_UTF8(*this->profileName), this->disableIMDS );
...
//        awsClientConfiguration.enableEndpointDiscovery = this->enableEndpointDiscovery;
        ...
        awsClientConfiguration.disableIMDS = this->disableIMDS;
        awsClientConfiguration.requestCompressionConfig = this->requestCompressionConfig.toAWS();
