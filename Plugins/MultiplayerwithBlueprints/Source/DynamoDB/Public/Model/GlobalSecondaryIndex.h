...
public:
    ...
    FGlobalSecondaryIndex &fromAWS(const Aws::DynamoDB::Model::GlobalSecondaryIndex &awsGlobalSecondaryIndex) {
        this->indexName = UTF8_TO_TCHAR(awsGlobalSecondaryIndex.GetIndexName().c_str());

        for (const Aws::DynamoDB::Model::KeySchemaElement& elem : awsGlobalSecondaryIndex.GetKeySchema()) {
            this->keySchema.Add(FDynamoDBKeySchemaElement().fromAWS(elem));
        }

        this->projection.fromAWS(awsGlobalSecondaryIndex.GetProjection());

        this->provisionedThroughput.fromAWS(awsGlobalSecondaryIndex.GetProvisionedThroughput());
