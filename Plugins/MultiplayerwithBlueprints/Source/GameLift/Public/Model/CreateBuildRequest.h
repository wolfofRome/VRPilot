...
public:
    Aws::GameLift::Model::CreateBuildRequest toAWS() const {
		...
		if (!(this->serverSdkVersion.IsEmpty())) {
            awsCreateBuildRequest.SetServerSdkVersion(TCHAR_TO_UTF8(*this->serverSdkVersion));
        }
    ...
    bool IsEmpty() const {
        return this->name.IsEmpty() && this->version.IsEmpty() && this->storageLocation.IsEmpty() && this->operatingSystem == EOperatingSystem::NOT_SET && this->tags.Num() == 0 && this->serverSdkVersion.IsEmpty();
    }
#endif
};
