...
public:
    Aws::GameLift::Model::UpdateFleetAttributesRequest toAWS() const {
        ...
        if (!(this->anywhereConfiguration.IsEmpty())) {
            awsUpdateFleetAttributesRequest.SetAnywhereConfiguration(this->anywhereConfiguration.toAWS());
        }
    ...
    bool IsEmpty() const {
        return this->fleetId.IsEmpty() && this->name.IsEmpty() && this->description.IsEmpty() && this->newGameSessionProtectionPolicy == EProtectionPolicy::NOT_SET && this->resourceCreationLimitPolicy.IsEmpty() && this->metricGroups.Num() == 0 && this->anywhereConfiguration.IsEmpty();
    }
#endif
};
