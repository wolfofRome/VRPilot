...
#include "AWSLambdaCreateFunctionUrlConfigRequest.generated.h"

USTRUCT(BlueprintType)
struct FAWSLambdaCreateFunctionUrlConfigRequest {
GENERATED_BODY()
...
#if WITH_LAMBDACLIENTSDK && WITH_CORE
public:
    Aws::Lambda::Model::CreateFunctionUrlConfigRequest toAWS() const {
        Aws::Lambda::Model::CreateFunctionUrlConfigRequest awsCreateFunctionUrlConfigRequest;

        if (!(this->functionName.IsEmpty())) {
            awsCreateFunctionUrlConfigRequest.SetFunctionName(TCHAR_TO_UTF8(*this->functionName));
        }

        if (!(this->qualifier.IsEmpty())) {
            awsCreateFunctionUrlConfigRequest.SetQualifier(TCHAR_TO_UTF8(*this->qualifier));
        }

        switch(this->authType) {
            case EAWSLambdaFunctionUrlAuthType::NONE:
                awsCreateFunctionUrlConfigRequest.SetAuthType(Aws::Lambda::Model::FunctionUrlAuthType::NONE);
                break;
            case EAWSLambdaFunctionUrlAuthType::AWS_IAM:
                awsCreateFunctionUrlConfigRequest.SetAuthType(Aws::Lambda::Model::FunctionUrlAuthType::AWS_IAM);
                break;
            default:
                break;
        }
        ...
        if (!(this->cors.IsEmpty())) {
            awsCreateFunctionUrlConfigRequest.SetCors(this->cors.toAWS());
        }

        return awsCreateFunctionUrlConfigRequest;
    }
