...
#include "GetLayerVersionResult.generated.h"
...
#if WITH_LAMBDACLIENTSDK && WITH_CORE
public:
    FGetLayerVersionResult &fromAWS(const Aws::Lambda::Model::GetLayerVersionResult &awsGetLayerVersionResult) {
        this->content.fromAWS(awsGetLayerVersionResult.GetContent());

        this->layerArn = UTF8_TO_TCHAR(awsGetLayerVersionResult.GetLayerArn().c_str());

        this->layerVersionArn = UTF8_TO_TCHAR(awsGetLayerVersionResult.GetLayerVersionArn().c_str());

        this->description = UTF8_TO_TCHAR(awsGetLayerVersionResult.GetDescription().c_str());

        this->createdDate = UTF8_TO_TCHAR(awsGetLayerVersionResult.GetCreatedDate().c_str());

        this->version = (int64)awsGetLayerVersionResult.GetVersion();

        this->compatibleRuntimes.Empty();
        for (const Aws::Lambda::Model::Runtime& elem : awsGetLayerVersionResult.GetCompatibleRuntimes()) {
            switch(elem) {
        ...
        this->licenseInfo = UTF8_TO_TCHAR(awsGetLayerVersionResult.GetLicenseInfo().c_str());

        this->compatibleArchitectures.Empty();
        for (const Aws::Lambda::Model::Architecture& elem : awsGetLayerVersionResult.GetCompatibleArchitectures()) {
            switch(elem) {
