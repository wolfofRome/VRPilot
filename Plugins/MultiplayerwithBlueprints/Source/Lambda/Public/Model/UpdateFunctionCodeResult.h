...
#include "UpdateFunctionCodeResult.generated.h"
...
#if WITH_LAMBDACLIENTSDK && WITH_CORE
public:
    FUpdateFunctionCodeResult &fromAWS(const Aws::Lambda::Model::UpdateFunctionCodeResult &awsUpdateFunctionCodeResult) {
        this->functionName = UTF8_TO_TCHAR(awsUpdateFunctionCodeResult.GetFunctionName().c_str());

        this->functionArn = UTF8_TO_TCHAR(awsUpdateFunctionCodeResult.GetFunctionArn().c_str());

        switch(awsUpdateFunctionCodeResult.GetRuntime()) {
        ...
        this->role = UTF8_TO_TCHAR(awsUpdateFunctionCodeResult.GetRole().c_str());

        this->handler = UTF8_TO_TCHAR(awsUpdateFunctionCodeResult.GetHandler().c_str());

        this->codeSize = (int64)awsUpdateFunctionCodeResult.GetCodeSize();

        this->description = UTF8_TO_TCHAR(awsUpdateFunctionCodeResult.GetDescription().c_str());

		this->timeout = awsUpdateFunctionCodeResult.GetTimeout();

		this->memorySize = awsUpdateFunctionCodeResult.GetMemorySize();

        this->lastModified = UTF8_TO_TCHAR(awsUpdateFunctionCodeResult.GetLastModified().c_str());

        this->codeSha256 = UTF8_TO_TCHAR(awsUpdateFunctionCodeResult.GetCodeSha256().c_str());

        this->version = UTF8_TO_TCHAR(awsUpdateFunctionCodeResult.GetVersion().c_str());

        this->vpcConfig.fromAWS(awsUpdateFunctionCodeResult.GetVpcConfig());

        this->deadLetterConfig.fromAWS(awsUpdateFunctionCodeResult.GetDeadLetterConfig());

        this->environment.fromAWS(awsUpdateFunctionCodeResult.GetEnvironment());

        this->kMSKeyArn = UTF8_TO_TCHAR(awsUpdateFunctionCodeResult.GetKMSKeyArn().c_str());

        this->tracingConfig.fromAWS(awsUpdateFunctionCodeResult.GetTracingConfig());

        this->masterArn = UTF8_TO_TCHAR(awsUpdateFunctionCodeResult.GetMasterArn().c_str());

        this->revisionId = UTF8_TO_TCHAR(awsUpdateFunctionCodeResult.GetRevisionId().c_str());

        this->layers.Empty();
        for (const Aws::Lambda::Model::Layer& elem : awsUpdateFunctionCodeResult.GetLayers()) {
            this->layers.Add(FAWSLayer().fromAWS(elem));
        }

        switch(awsUpdateFunctionCodeResult.GetState()) {
        ...
        this->stateReason = UTF8_TO_TCHAR(awsUpdateFunctionCodeResult.GetStateReason().c_str());

        switch(awsUpdateFunctionCodeResult.GetStateReasonCode()) {
        ...
        switch(awsUpdateFunctionCodeResult.GetLastUpdateStatus()) {
        ...
        this->lastUpdateStatusReason = UTF8_TO_TCHAR(awsUpdateFunctionCodeResult.GetLastUpdateStatusReason().c_str());

        switch(awsUpdateFunctionCodeResult.GetLastUpdateStatusReasonCode()) {
        ...
        for (const Aws::Lambda::Model::FileSystemConfig& elem : awsUpdateFunctionCodeResult.GetFileSystemConfigs()) {
            this->fileSystemConfigs.Add(FFileSystemConfig().fromAWS(elem));
        }

        switch(awsUpdateFunctionCodeResult.GetPackageType()) {
        ...
        this->imageConfigResponse.fromAWS(awsUpdateFunctionCodeResult.GetImageConfigResponse());

        this->signingProfileVersionArn = UTF8_TO_TCHAR(awsUpdateFunctionCodeResult.GetSigningProfileVersionArn().c_str());

        this->signingJobArn = UTF8_TO_TCHAR(awsUpdateFunctionCodeResult.GetSigningJobArn().c_str());

        this->architectures.Empty();
        for (const Aws::Lambda::Model::Architecture& elem : awsUpdateFunctionCodeResult.GetArchitectures()) {
            switch(elem) {
        ...
        this->ephemeralStorage.fromAWS(awsUpdateFunctionCodeResult.GetEphemeralStorage());

        this->snapStart.fromAWS(awsUpdateFunctionCodeResult.GetSnapStart());
