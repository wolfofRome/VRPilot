...
#include "CreateFunctionResult.generated.h"
...
#if WITH_LAMBDACLIENTSDK && WITH_CORE
public:
    FCreateFunctionResult &fromAWS(const Aws::Lambda::Model::CreateFunctionResult &awsCreateFunctionResult) {
        this->functionName = UTF8_TO_TCHAR(awsCreateFunctionResult.GetFunctionName().c_str());

        this->functionArn = UTF8_TO_TCHAR(awsCreateFunctionResult.GetFunctionArn().c_str());

        switch(awsCreateFunctionResult.GetRuntime()) {
        ...
        this->role = UTF8_TO_TCHAR(awsCreateFunctionResult.GetRole().c_str());

        this->handler = UTF8_TO_TCHAR(awsCreateFunctionResult.GetHandler().c_str());

        this->codeSize = (int64)awsCreateFunctionResult.GetCodeSize();

        this->description = UTF8_TO_TCHAR(awsCreateFunctionResult.GetDescription().c_str());

		this->timeout = awsCreateFunctionResult.GetTimeout();

		this->memorySize = awsCreateFunctionResult.GetMemorySize();

        this->lastModified = UTF8_TO_TCHAR(awsCreateFunctionResult.GetLastModified().c_str());

        this->codeSha256 = UTF8_TO_TCHAR(awsCreateFunctionResult.GetCodeSha256().c_str());

        this->version = UTF8_TO_TCHAR(awsCreateFunctionResult.GetVersion().c_str());

        this->vpcConfig.fromAWS(awsCreateFunctionResult.GetVpcConfig());

        this->deadLetterConfig.fromAWS(awsCreateFunctionResult.GetDeadLetterConfig());

        this->environment.fromAWS(awsCreateFunctionResult.GetEnvironment());

        this->kMSKeyArn = UTF8_TO_TCHAR(awsCreateFunctionResult.GetKMSKeyArn().c_str());

        this->tracingConfig.fromAWS(awsCreateFunctionResult.GetTracingConfig());

        this->masterArn = UTF8_TO_TCHAR(awsCreateFunctionResult.GetMasterArn().c_str());

        this->revisionId = UTF8_TO_TCHAR(awsCreateFunctionResult.GetRevisionId().c_str());

        this->layers.Empty();
        for (const Aws::Lambda::Model::Layer& elem : awsCreateFunctionResult.GetLayers()) {
            this->layers.Add(FAWSLayer().fromAWS(elem));
        }

        switch(awsCreateFunctionResult.GetState()) {
        ...
        this->stateReason = UTF8_TO_TCHAR(awsCreateFunctionResult.GetStateReason().c_str());

        switch(awsCreateFunctionResult.GetStateReasonCode()) {
        ...
        switch(awsCreateFunctionResult.GetLastUpdateStatus()) {
        ...
        this->lastUpdateStatusReason = UTF8_TO_TCHAR(awsCreateFunctionResult.GetLastUpdateStatusReason().c_str());

        switch(awsCreateFunctionResult.GetLastUpdateStatusReasonCode()) {
        ...
        for (const Aws::Lambda::Model::FileSystemConfig& elem : awsCreateFunctionResult.GetFileSystemConfigs()) {
            this->fileSystemConfigs.Add(FFileSystemConfig().fromAWS(elem));
        }

        switch(awsCreateFunctionResult.GetPackageType()) {
        ...
        this->imageConfigResponse.fromAWS(awsCreateFunctionResult.GetImageConfigResponse());

        this->signingProfileVersionArn = UTF8_TO_TCHAR(awsCreateFunctionResult.GetSigningProfileVersionArn().c_str());

        this->signingJobArn = UTF8_TO_TCHAR(awsCreateFunctionResult.GetSigningJobArn().c_str());

        this->architectures.Empty();
        for (const Aws::Lambda::Model::Architecture& elem : awsCreateFunctionResult.GetArchitectures()) {
            switch(elem) {
        ...
        this->ephemeralStorage.fromAWS(awsCreateFunctionResult.GetEphemeralStorage());

        this->snapStart.fromAWS(awsCreateFunctionResult.GetSnapStart());
