...
#include "AWSLambdaGetFunctionUrlConfigResult.generated.h"

USTRUCT(BlueprintType)
struct FAWSLambdaGetFunctionUrlConfigResult {
GENERATED_BODY()
...
#if WITH_LAMBDACLIENTSDK && WITH_CORE
public:
    FAWSLambdaGetFunctionUrlConfigResult &fromAWS(const Aws::Lambda::Model::GetFunctionUrlConfigResult &awsGetFunctionUrlConfigResult) {
        this->functionUrl = UTF8_TO_TCHAR(awsGetFunctionUrlConfigResult.GetFunctionUrl().c_str());

        this->functionArn = UTF8_TO_TCHAR(awsGetFunctionUrlConfigResult.GetFunctionArn().c_str());

        switch(awsGetFunctionUrlConfigResult.GetAuthType()) {
        ...
        this->cors.fromAWS(awsGetFunctionUrlConfigResult.GetCors());
        
        this->creationTime = UTF8_TO_TCHAR(awsGetFunctionUrlConfigResult.GetCreationTime().c_str());

        this->lastModifiedTime = UTF8_TO_TCHAR(awsGetFunctionUrlConfigResult.GetLastModifiedTime().c_str());
