...
#include "UpdateFunctionEventInvokeConfigRequest.generated.h"
...
#if WITH_LAMBDACLIENTSDK && WITH_CORE
public:
    Aws::Lambda::Model::UpdateFunctionEventInvokeConfigRequest toAWS() const {
        Aws::Lambda::Model::UpdateFunctionEventInvokeConfigRequest awsUpdateFunctionEventInvokeConfigRequest;

		if (!(this->functionName.IsEmpty())) {
            awsUpdateFunctionEventInvokeConfigRequest.SetFunctionName(TCHAR_TO_UTF8(*this->functionName));
        }

		if (!(this->qualifier.IsEmpty())) {
            awsUpdateFunctionEventInvokeConfigRequest.SetQualifier(TCHAR_TO_UTF8(*this->qualifier));
        }

        if (!(this->maximumRetryAttempts == 0)) {
            awsUpdateFunctionEventInvokeConfigRequest.SetMaximumRetryAttempts(this->maximumRetryAttempts);
        }

        if (!(this->maximumEventAgeInSeconds == 0)) {
            awsUpdateFunctionEventInvokeConfigRequest.SetMaximumEventAgeInSeconds(this->maximumEventAgeInSeconds);
        }

        if (!(this->destinationConfig.IsEmpty())) {
            awsUpdateFunctionEventInvokeConfigRequest.SetDestinationConfig(this->destinationConfig.toAWS());
        }

        return awsUpdateFunctionEventInvokeConfigRequest;
    }
