...
#include "UpdateFunctionConfigurationResult.generated.h"
...
#if WITH_LAMBDACLIENTSDK && WITH_CORE
public:
    FUpdateFunctionConfigurationResult &fromAWS(const Aws::Lambda::Model::UpdateFunctionConfigurationResult &awsUpdateFunctionConfigurationResult) {
        this->functionName = UTF8_TO_TCHAR(awsUpdateFunctionConfigurationResult.GetFunctionName().c_str());

        this->functionArn = UTF8_TO_TCHAR(awsUpdateFunctionConfigurationResult.GetFunctionArn().c_str());

        switch(awsUpdateFunctionConfigurationResult.GetRuntime()) {
        ...
        this->role = UTF8_TO_TCHAR(awsUpdateFunctionConfigurationResult.GetRole().c_str());

        this->handler = UTF8_TO_TCHAR(awsUpdateFunctionConfigurationResult.GetHandler().c_str());

        this->codeSize = (int64)awsUpdateFunctionConfigurationResult.GetCodeSize();

        this->description = UTF8_TO_TCHAR(awsUpdateFunctionConfigurationResult.GetDescription().c_str());

		this->timeout = awsUpdateFunctionConfigurationResult.GetTimeout();

		this->memorySize = awsUpdateFunctionConfigurationResult.GetMemorySize();

        this->lastModified = UTF8_TO_TCHAR(awsUpdateFunctionConfigurationResult.GetLastModified().c_str());

        this->codeSha256 = UTF8_TO_TCHAR(awsUpdateFunctionConfigurationResult.GetCodeSha256().c_str());

        this->version = UTF8_TO_TCHAR(awsUpdateFunctionConfigurationResult.GetVersion().c_str());

        this->vpcConfig.fromAWS(awsUpdateFunctionConfigurationResult.GetVpcConfig());
        
        this->deadLetterConfig.fromAWS(awsUpdateFunctionConfigurationResult.GetDeadLetterConfig());

        this->environment.fromAWS(awsUpdateFunctionConfigurationResult.GetEnvironment());

        this->kMSKeyArn = UTF8_TO_TCHAR(awsUpdateFunctionConfigurationResult.GetKMSKeyArn().c_str());

        this->tracingConfig.fromAWS(awsUpdateFunctionConfigurationResult.GetTracingConfig());

        this->masterArn = UTF8_TO_TCHAR(awsUpdateFunctionConfigurationResult.GetMasterArn().c_str());

        this->revisionId = UTF8_TO_TCHAR(awsUpdateFunctionConfigurationResult.GetRevisionId().c_str());

        this->layers.Empty();
        for (const Aws::Lambda::Model::Layer& elem : awsUpdateFunctionConfigurationResult.GetLayers()) {
            this->layers.Add(FAWSLayer().fromAWS(elem));
        }

        switch(awsUpdateFunctionConfigurationResult.GetState()) {
        ...
        this->stateReason = UTF8_TO_TCHAR(awsUpdateFunctionConfigurationResult.GetStateReason().c_str());

        switch(awsUpdateFunctionConfigurationResult.GetStateReasonCode()) {
        ...
        switch(awsUpdateFunctionConfigurationResult.GetLastUpdateStatus()) {
        ...
        this->lastUpdateStatusReason = UTF8_TO_TCHAR(awsUpdateFunctionConfigurationResult.GetLastUpdateStatusReason().c_str());

        switch(awsUpdateFunctionConfigurationResult.GetLastUpdateStatusReasonCode()) {
        ...
        for (const Aws::Lambda::Model::FileSystemConfig& elem : awsUpdateFunctionConfigurationResult.GetFileSystemConfigs()) {
            this->fileSystemConfigs.Add(FFileSystemConfig().fromAWS(elem));
        }

        switch(awsUpdateFunctionConfigurationResult.GetPackageType()) {
        ...
        this->imageConfigResponse.fromAWS(awsUpdateFunctionConfigurationResult.GetImageConfigResponse());

        this->signingProfileVersionArn = UTF8_TO_TCHAR(awsUpdateFunctionConfigurationResult.GetSigningProfileVersionArn().c_str());

        this->signingJobArn = UTF8_TO_TCHAR(awsUpdateFunctionConfigurationResult.GetSigningJobArn().c_str());

        this->architectures.Empty();
        for (const Aws::Lambda::Model::Architecture& elem : awsUpdateFunctionConfigurationResult.GetArchitectures()) {
            switch(elem) {
        ...
        this->ephemeralStorage.fromAWS(awsUpdateFunctionConfigurationResult.GetEphemeralStorage());

        this->snapStart.fromAWS(awsUpdateFunctionConfigurationResult.GetSnapStart());
