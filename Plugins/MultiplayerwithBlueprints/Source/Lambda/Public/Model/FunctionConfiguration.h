...
#include "FunctionConfiguration.generated.h"
...
#if WITH_LAMBDACLIENTSDK && WITH_CORE
public:
    FFunctionConfiguration &fromAWS(const Aws::Lambda::Model::FunctionConfiguration &awsFunctionConfiguration) {
        this->functionName = UTF8_TO_TCHAR(awsFunctionConfiguration.GetFunctionName().c_str());

        this->functionArn = UTF8_TO_TCHAR(awsFunctionConfiguration.GetFunctionArn().c_str());

        switch(awsFunctionConfiguration.GetRuntime()) {
        ...
        this->role = UTF8_TO_TCHAR(awsFunctionConfiguration.GetRole().c_str());

        this->handler = UTF8_TO_TCHAR(awsFunctionConfiguration.GetHandler().c_str());

        this->codeSize = (int64)awsFunctionConfiguration.GetCodeSize();

        this->description = UTF8_TO_TCHAR(awsFunctionConfiguration.GetDescription().c_str());

		this->timeout = awsFunctionConfiguration.GetTimeout();

		this->memorySize = awsFunctionConfiguration.GetMemorySize();

        this->lastModified = UTF8_TO_TCHAR(awsFunctionConfiguration.GetLastModified().c_str());

        this->codeSha256 = UTF8_TO_TCHAR(awsFunctionConfiguration.GetCodeSha256().c_str());

        this->version = UTF8_TO_TCHAR(awsFunctionConfiguration.GetVersion().c_str());

        this->vpcConfig.fromAWS(awsFunctionConfiguration.GetVpcConfig());

        this->deadLetterConfig.fromAWS(awsFunctionConfiguration.GetDeadLetterConfig());

        this->environment.fromAWS(awsFunctionConfiguration.GetEnvironment());

        this->kMSKeyArn = UTF8_TO_TCHAR(awsFunctionConfiguration.GetKMSKeyArn().c_str());

        this->tracingConfig.fromAWS(awsFunctionConfiguration.GetTracingConfig());

        this->masterArn = UTF8_TO_TCHAR(awsFunctionConfiguration.GetMasterArn().c_str());

        this->revisionId = UTF8_TO_TCHAR(awsFunctionConfiguration.GetRevisionId().c_str());

        this->layers.Empty();
        for (const Aws::Lambda::Model::Layer& elem : awsFunctionConfiguration.GetLayers()) {
            this->layers.Add(FAWSLayer().fromAWS(elem));
        }

        switch(awsFunctionConfiguration.GetState()) {
        ...
        this->stateReason = UTF8_TO_TCHAR(awsFunctionConfiguration.GetStateReason().c_str());

        switch(awsFunctionConfiguration.GetStateReasonCode()) {
        ...
        switch(awsFunctionConfiguration.GetLastUpdateStatus()) {
        ...
        this->lastUpdateStatusReason = UTF8_TO_TCHAR(awsFunctionConfiguration.GetLastUpdateStatusReason().c_str());

        switch(awsFunctionConfiguration.GetLastUpdateStatusReasonCode()) {
        ...
        for (const Aws::Lambda::Model::FileSystemConfig& elem : awsFunctionConfiguration.GetFileSystemConfigs()) {
            this->fileSystemConfigs.Add(FFileSystemConfig().fromAWS(elem));
        }

        switch(awsFunctionConfiguration.GetPackageType()) {
        ...
        this->imageConfigResponse.fromAWS(awsFunctionConfiguration.GetImageConfigResponse());

        this->signingProfileVersionArn = UTF8_TO_TCHAR(awsFunctionConfiguration.GetSigningProfileVersionArn().c_str());

        this->signingJobArn = UTF8_TO_TCHAR(awsFunctionConfiguration.GetSigningJobArn().c_str());

        this->architectures.Empty();
        for (const Aws::Lambda::Model::Architecture& elem : awsFunctionConfiguration.GetArchitectures()) {
            switch(elem) {
        ...
        this->ephemeralStorage.fromAWS(awsFunctionConfiguration.GetEphemeralStorage());

        this->snapStart.fromAWS(awsFunctionConfiguration.GetSnapStart());
