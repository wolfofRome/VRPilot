...
#include "GetFunctionConfigurationResult.generated.h"
...
#if WITH_LAMBDACLIENTSDK && WITH_CORE
public:
    FGetFunctionConfigurationResult &fromAWS(const Aws::Lambda::Model::GetFunctionConfigurationResult &awsGetFunctionConfigurationResult) {
        this->functionName = UTF8_TO_TCHAR(awsGetFunctionConfigurationResult.GetFunctionName().c_str());

        this->functionArn = UTF8_TO_TCHAR(awsGetFunctionConfigurationResult.GetFunctionArn().c_str());

        switch(awsGetFunctionConfigurationResult.GetRuntime()) {
        ...
        this->role = UTF8_TO_TCHAR(awsGetFunctionConfigurationResult.GetRole().c_str());

        this->handler = UTF8_TO_TCHAR(awsGetFunctionConfigurationResult.GetHandler().c_str());

        this->codeSize = (int64)awsGetFunctionConfigurationResult.GetCodeSize();

        this->description = UTF8_TO_TCHAR(awsGetFunctionConfigurationResult.GetDescription().c_str());

		this->timeout = awsGetFunctionConfigurationResult.GetTimeout();

		this->memorySize = awsGetFunctionConfigurationResult.GetMemorySize();

        this->lastModified = UTF8_TO_TCHAR(awsGetFunctionConfigurationResult.GetLastModified().c_str());

        this->codeSha256 = UTF8_TO_TCHAR(awsGetFunctionConfigurationResult.GetCodeSha256().c_str());

        this->version = UTF8_TO_TCHAR(awsGetFunctionConfigurationResult.GetVersion().c_str());

        this->vpcConfig.fromAWS(awsGetFunctionConfigurationResult.GetVpcConfig());

        this->deadLetterConfig.fromAWS(awsGetFunctionConfigurationResult.GetDeadLetterConfig());

        this->environment.fromAWS(awsGetFunctionConfigurationResult.GetEnvironment());

        this->kMSKeyArn = UTF8_TO_TCHAR(awsGetFunctionConfigurationResult.GetKMSKeyArn().c_str());

        this->tracingConfig.fromAWS(awsGetFunctionConfigurationResult.GetTracingConfig());

        this->masterArn = UTF8_TO_TCHAR(awsGetFunctionConfigurationResult.GetMasterArn().c_str());

        this->revisionId = UTF8_TO_TCHAR(awsGetFunctionConfigurationResult.GetRevisionId().c_str());

        this->layers.Empty();
        for (const Aws::Lambda::Model::Layer& elem : awsGetFunctionConfigurationResult.GetLayers()) {
            this->layers.Add(FAWSLayer().fromAWS(elem));
        }

        switch(awsGetFunctionConfigurationResult.GetState()) {
        ...
        this->stateReason = UTF8_TO_TCHAR(awsGetFunctionConfigurationResult.GetStateReason().c_str());

        switch(awsGetFunctionConfigurationResult.GetStateReasonCode()) {
        ...
        switch(awsGetFunctionConfigurationResult.GetLastUpdateStatus()) {
        ...
        this->lastUpdateStatusReason = UTF8_TO_TCHAR(awsGetFunctionConfigurationResult.GetLastUpdateStatusReason().c_str());

        switch(awsGetFunctionConfigurationResult.GetLastUpdateStatusReasonCode()) {
        ...
        for (const Aws::Lambda::Model::FileSystemConfig& elem : awsGetFunctionConfigurationResult.GetFileSystemConfigs()) {
            this->fileSystemConfigs.Add(FFileSystemConfig().fromAWS(elem));
        }

        switch(awsGetFunctionConfigurationResult.GetPackageType()) {
        ...
        this->imageConfigResponse.fromAWS(awsGetFunctionConfigurationResult.GetImageConfigResponse());

        this->signingProfileVersionArn = UTF8_TO_TCHAR(awsGetFunctionConfigurationResult.GetSigningProfileVersionArn().c_str());

        this->signingJobArn = UTF8_TO_TCHAR(awsGetFunctionConfigurationResult.GetSigningJobArn().c_str());

        this->architectures.Empty();
        for (const Aws::Lambda::Model::Architecture& elem : awsGetFunctionConfigurationResult.GetArchitectures()) {
            switch(elem) {
        ...
        this->ephemeralStorage.fromAWS(awsGetFunctionConfigurationResult.GetEphemeralStorage());

        this->snapStart.fromAWS(awsGetFunctionConfigurationResult.GetSnapStart());
