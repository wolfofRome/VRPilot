...
public:
    FEventSourceMappingConfiguration &fromAWS(const Aws::Lambda::Model::EventSourceMappingConfiguration &awsEventSourceMappingConfiguration) {
        ...
        switch(awsEventSourceMappingConfiguration.GetStartingPosition()) {
        ...
        this->startingPositionTimestamp = FDateTime(1970, 1, 1) + FTimespan(awsEventSourceMappingConfiguration.GetStartingPositionTimestamp().Millis() * ETimespan::TicksPerMillisecond);
		...
		this->parallelizationFactor = awsEventSourceMappingConfiguration.GetParallelizationFactor();

        this->eventSourceArn = UTF8_TO_TCHAR(awsEventSourceMappingConfiguration.GetEventSourceArn().c_str());

        this->filterCriteria.fromAWS(awsEventSourceMappingConfiguration.GetFilterCriteria());

        this->functionArn = UTF8_TO_TCHAR(awsEventSourceMappingConfiguration.GetFunctionArn().c_str());

        this->lastModified = FDateTime(1970, 1, 1) + FTimespan(awsEventSourceMappingConfiguration.GetLastModified().Millis() * ETimespan::TicksPerMillisecond);
        ...
        this->destinationConfig.fromAWS(awsEventSourceMappingConfiguration.GetDestinationConfig());

        this->topics.Empty();
        for (const Aws::String& elem : awsEventSourceMappingConfiguration.GetTopics()) {
            this->topics.Add(UTF8_TO_TCHAR(elem.c_str()));
        }

        this->queues.Empty();
        for (const Aws::String& elem : awsEventSourceMappingConfiguration.GetQueues()) {
            this->queues.Add(UTF8_TO_TCHAR(elem.c_str()));
        }

        this->sourceAccessConfigurations.Empty();
        for (const Aws::Lambda::Model::SourceAccessConfiguration& elem : awsEventSourceMappingConfiguration.GetSourceAccessConfigurations()) {
            this->sourceAccessConfigurations.Add(FAWSLambdaSourceAccessConfiguration().fromAWS(elem));
        }

        this->selfManagedEventSource.fromAWS(awsEventSourceMappingConfiguration.GetSelfManagedEventSource());

        this->maximumRecordAgeInSeconds = awsEventSourceMappingConfiguration.GetMaximumRecordAgeInSeconds();

        this->bisectBatchOnFunctionError = awsEventSourceMappingConfiguration.GetBisectBatchOnFunctionError();

		this->maximumRetryAttempts = awsEventSourceMappingConfiguration.GetMaximumRetryAttempts();

		this->tumblingWindowInSeconds = awsEventSourceMappingConfiguration.GetTumblingWindowInSeconds();

        this->functionResponseTypes.Empty();
        for (const Aws::Lambda::Model::FunctionResponseType& elem : awsEventSourceMappingConfiguration.GetFunctionResponseTypes()) {
            switch(elem) {
        ...
        this->amazonManagedKafkaEventSourceConfig.fromAWS(awsEventSourceMappingConfiguration.GetAmazonManagedKafkaEventSourceConfig());

        this->selfManagedKafkaEventSourceConfig.fromAWS(awsEventSourceMappingConfiguration.GetSelfManagedKafkaEventSourceConfig());
