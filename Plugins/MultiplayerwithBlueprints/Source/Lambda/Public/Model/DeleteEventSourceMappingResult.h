...
public:
    FDeleteEventSourceMappingResult &fromAWS(const Aws::Lambda::Model::DeleteEventSourceMappingResult &awsDeleteEventSourceMappingResult) {
        ...
        switch(awsDeleteEventSourceMappingResult.GetStartingPosition()) {
        ...
        this->startingPositionTimestamp = FDateTime(1970, 1, 1) + FTimespan(awsDeleteEventSourceMappingResult.GetStartingPositionTimestamp().Millis() * ETimespan::TicksPerMillisecond);
		...
		this->parallelizationFactor = awsDeleteEventSourceMappingResult.GetParallelizationFactor();

        this->eventSourceArn = UTF8_TO_TCHAR(awsDeleteEventSourceMappingResult.GetEventSourceArn().c_str());

        this->filterCriteria.fromAWS(awsDeleteEventSourceMappingResult.GetFilterCriteria());

        this->functionArn = UTF8_TO_TCHAR(awsDeleteEventSourceMappingResult.GetFunctionArn().c_str());

        this->lastModified = FDateTime(1970, 1, 1) + FTimespan(awsDeleteEventSourceMappingResult.GetLastModified().Millis() * ETimespan::TicksPerMillisecond);
        ...
        this->destinationConfig.fromAWS(awsDeleteEventSourceMappingResult.GetDestinationConfig());

        this->topics.Empty();
        for (const Aws::String& elem : awsDeleteEventSourceMappingResult.GetTopics()) {
            this->topics.Add(UTF8_TO_TCHAR(elem.c_str()));
        }

        this->queues.Empty();
        for (const Aws::String& elem : awsDeleteEventSourceMappingResult.GetQueues()) {
            this->queues.Add(UTF8_TO_TCHAR(elem.c_str()));
        }

        this->sourceAccessConfigurations.Empty();
        for (const Aws::Lambda::Model::SourceAccessConfiguration& elem : awsDeleteEventSourceMappingResult.GetSourceAccessConfigurations()) {
            this->sourceAccessConfigurations.Add(FAWSLambdaSourceAccessConfiguration().fromAWS(elem));
        }

        this->selfManagedEventSource.fromAWS(awsDeleteEventSourceMappingResult.GetSelfManagedEventSource());

		this->maximumRecordAgeInSeconds = awsDeleteEventSourceMappingResult.GetMaximumRecordAgeInSeconds();

        this->bisectBatchOnFunctionError = awsDeleteEventSourceMappingResult.GetBisectBatchOnFunctionError();

		this->maximumRetryAttempts = awsDeleteEventSourceMappingResult.GetMaximumRetryAttempts();

		this->tumblingWindowInSeconds = awsDeleteEventSourceMappingResult.GetTumblingWindowInSeconds();

        this->functionResponseTypes.Empty();
        for (const Aws::Lambda::Model::FunctionResponseType& elem : awsDeleteEventSourceMappingResult.GetFunctionResponseTypes()) {
            switch(elem) {
        ...
        this->amazonManagedKafkaEventSourceConfig.fromAWS(awsDeleteEventSourceMappingResult.GetAmazonManagedKafkaEventSourceConfig());

        this->selfManagedKafkaEventSourceConfig.fromAWS(awsDeleteEventSourceMappingResult.GetSelfManagedKafkaEventSourceConfig());
