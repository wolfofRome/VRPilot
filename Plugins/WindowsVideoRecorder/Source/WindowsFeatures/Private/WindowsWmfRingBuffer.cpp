...
DECLARE_LOG_CATEGORY_EXTERN(WindowsWmfRingBuffer, Log, VeryVerbose);
DEFINE_LOG_CATEGORY(WindowsWmfRingBuffer);

WINDOWSFEATURES_START

void FWindowsWmfRingBuffer::Push(AVEncoder::FMediaPacket&& Sample)
{
	check(MaxDuration != 0);
...
void FWindowsWmfRingBuffer::Cleanup()
// removes a/v samples grouped by the oldest key-frame period
{
	...
	UE_LOG(WindowsWmfRingBuffer, VeryVerbose, TEXT("%d samples, %.3f s, %.3f - %.3f:%.3f"),
		Samples.Num(),
...
void FWindowsWmfRingBuffer::PauseCleanup(bool bPause)
{
	bCleanupPaused = bPause;
}

FTimespan FWindowsWmfRingBuffer::GetDuration() const
{
	if (Samples.Num() == 0)
	{
		return 0;
...
TArray<AVEncoder::FMediaPacket> FWindowsWmfRingBuffer::GetCopy()
{
	FScopeLock Lock(&Mutex);
...
void FWindowsWmfRingBuffer::Reset()
{
	FScopeLock Lock(&Mutex);
	Samples.Empty();
}
