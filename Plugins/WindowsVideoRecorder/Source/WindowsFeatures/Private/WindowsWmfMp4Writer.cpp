...
DECLARE_LOG_CATEGORY_EXTERN(WindowsMP4, Log, VeryVerbose);

DEFINE_LOG_CATEGORY(WindowsMP4);

WINDOWSFEATURES_START

bool FWindowsWmfMp4Writer::Initialize(const TCHAR* Filename)
{
	CHECK_HR(MFCreateSinkWriterFromURL(Filename, nullptr, nullptr, Writer.GetInitReference()));
	UE_LOG(WindowsWMF, Verbose, TEXT("Initialised Mp4Writer for %s"), Filename);
	return true;
}

TOptional<DWORD> FWindowsWmfMp4Writer::CreateAudioStream(const FString& Codec, const AVEncoder::FAudioConfig& Config)
{
	GUID Format;
	...
	{
		UE_LOG(WindowsWMF, Error, TEXT("FWindowsWmfMp4Writer doesn't support codec %s"), *Codec);
		return {};
	}
...
TOptional<DWORD> FWindowsWmfMp4Writer::CreateVideoStream(const FString& Codec, const AVEncoder::FVideoConfig& Config)
{
	GUID Format;
	if (Codec == "h264")
	{
...
bool FWindowsWmfMp4Writer::Start()
{
	CHECK_HR(Writer->BeginWriting());
	return true;
}

bool FWindowsWmfMp4Writer::Write(const AVEncoder::FMediaPacket& InSample, DWORD StreamIndex)
{
	...
	UE_LOG(WindowsMP4, VeryVerbose, TEXT("stream #%d: time %.3f, duration %.3f%s"), StreamIndex, Sample.GetTime().GetTotalSeconds(), Sample.GetDuration().GetTotalSeconds(), InSample.IsVideoKeyFrame() ? TEXT(", key-frame") : TEXT(""));

	return true;
}

bool FWindowsWmfMp4Writer::Finalize()
{
	CHECK_HR(Writer->Finalize());
	UE_LOG(WindowsWMF, VeryVerbose, TEXT("Closed .mp4"));
