...
DEFINE_LOG_CATEGORY(WinVideoRecordingSystem);
//CSV_DEFINE_CATEGORY(WinVideoRecordingSystem, true);
 ...
 */
class FWinVideoRecordingSystem::FWindowsScreenRecording
{
public:
	bool Disable()
	{
...
FWinVideoRecordingSystem::FWinVideoRecordingSystem()
{
	UE_LOG(WinVideoRecordingSystem, Verbose, TEXT("%s"), __FUNCTIONW__);
...
void FWinVideoRecordingSystem::EnableRecording(bool bEnableRecording)
{
	//SCOPE_CYCLE_COUNTER(STAT_WinVideoRecordingSystem_EnableRecording);
	//CSV_SCOPED_TIMING_STAT(WinVideoRecordingSystem, EnableRecording);

	if (!FApp::CanEverRender())
	{
		UE_LOG(WinVideoRecordingSystem, Warning, TEXT("Can't enable recording because this App can't render."));
		return;
	}
	...
	{
		Recorder.Reset(new FWindowsHighlightRecorder());
	}
...
bool FWinVideoRecordingSystem::IsEnabled() const
{
	UE_LOG(WinVideoRecordingSystem, Verbose, TEXT("%s"), __FUNCTIONW__);
	return Recorder.Get()==nullptr ? false : true;
}

void FWinVideoRecordingSystem::NextRecording()
{
...
bool FWinVideoRecordingSystem::NewRecording(const TCHAR* DestinationFileName, FRecordingParameters InParameters, FVideoRecordingConfig Config)
{
	//SCOPE_CYCLE_COUNTER(STAT_WinVideoRecordingSystem_NewRecording);
	//CSV_SCOPED_TIMING_STAT(WinVideoRecordingSystem, NewRecording);

	UE_LOG(WinVideoRecordingSystem, Verbose, TEXT("%s"), __FUNCTIONW__);

	if (!IsEnabled())
	{
		UE_LOG(WinVideoRecordingSystem, Warning, TEXT("%s : can't open a new recording. Recording is disabled"), __FUNCTIONW__);
		return false;
	}

	if (RecordState != ERecordingState::None)
	{
		UE_LOG(WinVideoRecordingSystem, Warning, TEXT("FWinVideoRecordingSystem::NewRecording: can't open a new recording, one is already in progress."));
		return false;
	}

	
	Parameters = InParameters;
	VideoRecordingConfigs = Config;
	...
	if (Recorder->GetState() == FWindowsHighlightRecorder::EState::Stopped)
	{
		// Call Start to initialize the internals, and pause right away
		if (Recorder->InitializeVideoRecordingConfig(VideoRecordingConfigs) == false || Recorder->Start(Parameters.RecordingLengthSeconds) == false)
		{
	...
	check(bPauseRet);
	RecordState = ERecordingState::Paused;
...
void FWinVideoRecordingSystem::StartRecording()
{
	//SCOPE_CYCLE_COUNTER(STAT_WinVideoRecordingSystem_StartRecording);
	//CSV_SCOPED_TIMING_STAT(WinVideoRecordingSystem, StartRecording);

	UE_LOG(WinVideoRecordingSystem, Verbose, TEXT("%s"), __FUNCTIONW__);

	if (RecordState != ERecordingState::Paused)
	{
		UE_LOG(WinVideoRecordingSystem, Warning, TEXT("%s: can't start recording, invalid state"), __FUNCTIONW__);
		return;
	}

	UE_LOG(WinVideoRecordingSystem, Log, TEXT("%s: starting a recording"), __FUNCTIONW__);

	if (Recorder->GetState() == FWindowsHighlightRecorder::EState::Stopped)
	{
		Recorder->Start(Parameters.RecordingLengthSeconds);
	}
	if (Recorder->GetState() == FWindowsHighlightRecorder::EState::Paused)
	{
		bool bPauseRet = Recorder->Pause(false);
		check(bPauseRet);
	}

	RecordState = ERecordingState::Recording;
	CurrentStartRecordingCycles = FPlatformTime::Cycles64();
}

void FWinVideoRecordingSystem::PauseRecording()
{
	//SCOPE_CYCLE_COUNTER(STAT_WinVideoRecordingSystem_PauseRecording);
	//CSV_SCOPED_TIMING_STAT(WinVideoRecordingSystem, PauseRecording);

	UE_LOG(WinVideoRecordingSystem, Verbose, TEXT("%s"), __FUNCTIONW__);

	if (RecordState != ERecordingState::Recording)
	{
		UE_LOG(WinVideoRecordingSystem, Warning, TEXT("%s: can't pause recording, invalid state."), __FUNCTIONW__);
		return;
	}

	UE_LOG(WinVideoRecordingSystem, Log, TEXT("%s: pausing a recording"), __FUNCTIONW__);
...
uint64 FWinVideoRecordingSystem::GetMinimumRecordingSeconds() const
{
	return 1;
}

uint64 FWinVideoRecordingSystem::GetMaximumRecordingSeconds() const
{
	return 90000;
}

float FWinVideoRecordingSystem::GetCurrentRecordingSeconds() const
{
	float Ret = (FPlatformTime::Cycles64() - CurrentStartRecordingCycles + CyclesBeforePausing) * FPlatformTime::GetSecondsPerCycle();
	UE_LOG(WinVideoRecordingSystem, Verbose, TEXT("%s: reporting %f"), __FUNCTIONW__, Ret);
	return Ret;
}

// #RVF : Make use of Comment ?
void FWinVideoRecordingSystem::FinalizeRecording(const bool bSaveRecording, const FText& Title, const FText& Comment, const bool bStopAutoContinue/* = true*/)
{
	//SCOPE_CYCLE_COUNTER(STAT_WinVideoRecordingSystem_FinalizeRecording);
	//CSV_SCOPED_TIMING_STAT(WinVideoRecordingSystem, FinalizeRecording);

	UE_LOG(WinVideoRecordingSystem, Verbose, TEXT("%s"), __FUNCTIONW__);

	if (RecordState == ERecordingState::None)
	{
		UE_LOG(WinVideoRecordingSystem, Warning, TEXT("%s: can't finalize recording, invalid state."), __FUNCTIONW__);
		return;
	}
	...
	if (bSaveRecording)
	{
		bool bRet = Recorder->SaveHighlight(*CurrentFilename, [this, bStopAutoContinue](bool bRes, const FString& InFullPathToFile)
		{
			...
			FGraphEventRef FinalizeEvent = FSimpleDelegateGraphTask::CreateAndDispatchWhenReady(
				FSimpleDelegateGraphTask::FDelegate::CreateRaw(this, &FWinVideoRecordingSystem::FinalizeCallbackOnGameThread,
		...
		if (!bRet)
		{
			UE_LOG(WinVideoRecordingSystem, Warning, TEXT("%s: can't finalize recording."), __FUNCTIONW__);
			return;
		}

		RecordState = ERecordingState::Finalizing;
	}
	else
	{
		// Drop this recording
...
void FWinVideoRecordingSystem::FinalizeCallbackOnGameThread(bool bSaved, bool bAutoContinue, FString Path, bool bBroadcast)
{
	UE_LOG(WinVideoRecordingSystem, Verbose, TEXT("%s"), __FUNCTIONW__);
	RecordState = ERecordingState::None;
...
ERecordingState FWinVideoRecordingSystem::GetRecordingState() const
{
	return RecordState;
}

void FWinVideoRecordingSystem::InitializeVideoRecordingConfig(FVideoRecordingConfig InVideoRecordingConfig)
{
	VideoRecordingConfigs = InVideoRecordingConfig;
}
