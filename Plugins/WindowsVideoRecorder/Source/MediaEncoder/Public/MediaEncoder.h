...
#include "AvEncoder/Public/AudioEncoder.h"
...
class IMediaEncoderListener
{
public:
	virtual void OnMediaSample(const AVEncoder::FMediaPacket& Sample) = 0;
};

class MEDIAENCODER_API FMediaEncoder final : private ISubmixBufferListener, public AVEncoder::IAudioEncoderListener
{
public:
	 ...
	 * Get the singleton
	 */
	static FMediaEncoder* Get();

	~FMediaEncoder();

	bool RegisterListener(IMediaEncoderListener* Listener);
	void UnregisterListener(IMediaEncoderListener* Listener);

	void SetVideoBitrate(uint32 Bitrate);
	void SetVideoFramerate(uint32 Framerate);
	bool InitializeVideoConfig(uint32 Width, uint32 Height, uint32 Framerate, uint32 Bitrate, bool InIsAudio, bool InShowUI);
...
private:
	...
	TArray<IMediaEncoderListener*> Listeners;
	...
	friend class FMediaEncoderModule;
	static FMediaEncoder* Singleton;
