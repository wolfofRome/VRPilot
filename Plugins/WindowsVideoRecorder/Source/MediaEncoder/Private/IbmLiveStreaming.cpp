...
FAutoConsoleCommand LiveStreamingStart(
	...
	TEXT("Starts live streaming  to IBM Cloud Video"),
...
bool FIbmLiveStreaming::Start()
{
	...
	FMediaEncoder* MediaEncoder = FMediaEncoder::Get();
	// First thing we need to do is call RegisterListener, which initializes FMediaEncoder if this is the first listener
...
bool FIbmLiveStreaming::Start(const FString& ClientId, const FString& ClientSecret, const FString& Channel, uint32 AudioSampleRate, uint32 AudioNumChannels, uint32 AudioBitrate)
{
	...
	FMediaEncoder::Get()->RegisterListener(this);
...
void FIbmLiveStreaming::OnMediaSample(const FMediaEncoderSample& Sample)
{
...
void FIbmLiveStreaming::OnStreamBandwidthChangedImpl(RTMPModuleBroadcaster* Module, uint32 Bandwidth, bool bQueueWasEmpty)
{
	...
	verify(FMediaEncoder::Get()->GetVideoOutputType(VideoOutputType));
	...
	if (NewVideoBitrate)
	{
		FMediaEncoder::Get()->SetVideoBitrate(NewVideoBitrate);
	}

	if (NewVideoFramerate)
	{
		FMediaEncoder::Get()->SetVideoFramerate(NewVideoFramerate);
	}
