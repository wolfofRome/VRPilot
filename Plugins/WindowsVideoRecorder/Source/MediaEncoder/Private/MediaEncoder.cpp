...
DEFINE_LOG_CATEGORY(MediaEncoder);
CSV_DEFINE_CATEGORY(MediaEncoder, true);
...
const uint32 MaxWidth = 3840;
const uint32 MaxHeight = 2160;

FAutoConsoleCommand MediaEncoderInitialize(TEXT("MediaEncoder.Initialize"), TEXT("Constructs the audio/video encoding objects. Does not start encoding"),
                                                   FConsoleCommandDelegate::CreateStatic(&FMediaEncoder::InitializeCmd));

FAutoConsoleCommand MediaEncoderStart(TEXT("MediaEncoder.Start"), TEXT("Starts encoding"), FConsoleCommandDelegate::CreateStatic(&FMediaEncoder::StartCmd));

FAutoConsoleCommand MediaEncoderStop(TEXT("MediaEncoder.Stop"), TEXT("Stops encoding"), FConsoleCommandDelegate::CreateStatic(&FMediaEncoder::StopCmd));

FAutoConsoleCommand MediaEncoderShutdown(TEXT("MediaEncoder.Shutdown"), TEXT("Releases all systems."), FConsoleCommandDelegate::CreateStatic(&FMediaEncoder::ShutdownCmd));
...
FMediaEncoder* FMediaEncoder::Singleton = nullptr;

FMediaEncoder* FMediaEncoder::Get()
{
	if(!Singleton)
	{
		Singleton = new FMediaEncoder();
	}
	return Singleton;
}

FMediaEncoder::FMediaEncoder() {}

FMediaEncoder::~FMediaEncoder() { Shutdown(); }

bool FMediaEncoder::RegisterListener(IMediaEncoderListener* Listener)
{
	...
	if(Listeners.Num() == 0)
	{
		UE_LOG(MediaEncoder, Log, TEXT("Registering the first listener"));
		if(!Start())
		{
			return false;
		}
...
void FMediaEncoder::UnregisterListener(IMediaEncoderListener* Listener)
{
	check(IsInGameThread());
	...
	{
		UE_LOG(MediaEncoder, Log, TEXT("Unregistered the last listener"));
		Stop();
	}
}

bool FMediaEncoder::Initialize()
{
	//MemoryCheckpoint("Initial");

	if(VideoEncoder)
	{
		UE_LOG(MediaEncoder, Log, TEXT("Already initialized"));
		return true;
	}
	...
	{
		UE_LOG(MediaEncoder, Error, TEXT("No audio encoder for aac found"));
		return false;
	}
	...
	if(!AudioEncoder)
	{
		UE_LOG(MediaEncoder, Error, TEXT("Could not create audio encoder"));
		return false;
	}
	...
	{
		UE_LOG(MediaEncoder, Error, TEXT("Could not initialize audio encoder"));
		return false;
	}
	...
	//FParse::Value(FCommandLine::Get(), TEXT("MediaEncoder.ResY="), VideoConfig.Height);
	//UE_LOG(MediaEncoder, Log, TEXT("MediaEncoder.ResY = %d"), VideoConfig.Height);
	//if(VideoConfig.Height == 0 || VideoConfig.Height == 1080)
	//{
	//	VideoConfig.Width = 1920;
	//	VideoConfig.Height = 1080;
	//}
	//else if (VideoConfig.Height == 720)
	//{
	//	VideoConfig.Width = 1280;
	//	VideoConfig.Height = 720;
	//}
	//else if (VideoConfig.Height == 1440)
	//{
	//	VideoConfig.Width = 2560;
	//	VideoConfig.Height = 1440;
	//}
	//else if(VideoConfig.Height == 2160)
	//{
	//	VideoConfig.Width = 3840;
	//	VideoConfig.Height = 2160;
	//}
	//else if (VideoConfig.Height == 4360)
	//{
	//	VideoConfig.Width = 7680;
	//	VideoConfig.Height = 2160;
	//}
	//else
	//{
	//	UE_LOG(MediaEncoder, Fatal, TEXT("MediaEncoder.ResY can only have a value of 720 or 1080 or 1440 or 2160"));
	//	return false;
	//}

	//// Specifying 0 will completely disable frame skipping (therefore encoding as many frames as possible)
	//FParse::Value(FCommandLine::Get(), TEXT("MediaEncoder.FPS="), VideoConfig.Framerate);
	...
	//	UE_LOG(MediaEncoder, Log, TEXT("Uncapping FPS"));
	//}
	//else
	//{
	//	VideoConfig.Framerate = FMath::Clamp(VideoConfig.Framerate, (uint32)MinVideoFPS, (uint32)MaxVideoFPS);
	//	bDoFrameSkipping = true;
	//	UE_LOG(MediaEncoder, Log, TEXT("Capping FPS %u"), VideoConfig.Framerate);
	//}

	//VideoConfig.Bitrate = HardcodedVideoBitrate;
	//FParse::Value(FCommandLine::Get(), TEXT("MediaEncoder.Bitrate="), VideoConfig.Bitrate);
...
#endif
		{
			UE_LOG(MediaEncoder, Error, TEXT("Video encoding is not supported with the current Platform/RHI combo."));
			return false;
		}
	}
	...
	{
		UE_LOG(MediaEncoder, Error, TEXT("No video encoders found. Check if relevent encoder plugins have been enabled for this project."));
		return false;
	}
	...
	if (!VideoEncoder)
	{
		UE_LOG(MediaEncoder, Error, TEXT("No H264 video encoder found. Check if relevent encoder plugins have been enabled for this project."));
		return false;
	}
	...
	if(!VideoEncoder)
	{
		UE_LOG(MediaEncoder, Fatal, TEXT("Creating video encoder failed."));
		return false;
	}
...
bool FMediaEncoder::Start()
{
	if(StartTime != 0)
	{
		UE_LOG(MediaEncoder, Log, TEXT("Already running"));
		return true;
	}

	if(!VideoEncoder)
	{
		UE_LOG(MediaEncoder, Log, TEXT("Not initialized yet , so also performing a Intialize()"));
		if(!Initialize())
		{
			return false;
		}
	...
	FSlateApplication::Get().GetRenderer()->OnBackBufferReadyToPresent().AddRaw(this, &FMediaEncoder::OnFrameBufferReady);
	
	
	return true;
}

void FMediaEncoder::Stop()
{
	check(IsInGameThread());

	if(StartTime == 0)
	{
		UE_LOG(MediaEncoder, Log, TEXT("Not running"));
		return;
	}
...
AVEncoder::FAudioConfig FMediaEncoder::GetAudioConfig() const
{
	if(AudioEncoder)
	{
...
void FMediaEncoder::Shutdown()
{
	if(StartTime != 0)
	{
		UE_LOG(MediaEncoder, Log, TEXT("Currently running, so also performing a Stop()"));
		Stop();
	}
...
FTimespan FMediaEncoder::GetMediaTimestamp() const { return FTimespan::FromSeconds(FPlatformTime::Seconds()) - StartTime; }

void FMediaEncoder::OnNewSubmixBuffer(const USoundSubmix* OwningSubmix, float* AudioData, int32 NumSamples, int32 NumChannels, const int32 SampleRate, double /*AudioClock*/)
{
	CSV_SCOPED_TIMING_STAT(MediaEncoder, OnNewSubmixBuffer);
	if(SampleRate != HardcodedAudioSamplerate)
	{
		...
		if(!bAudioFormatChecked)
		{
			...
			UE_LOG(MediaEncoder, Error, TEXT("Audio SampleRate needs to be %d HZ, current value is %d. VideoRecordingSystem won't record audio"), HardcodedAudioSamplerate, SampleRate);
		}
		return;
	}
...
void FMediaEncoder::OnFrameBufferReady(SWindow& SlateWindow, const FTexture2DRHIRef& FrameBuffer)
{
	CSV_SCOPED_TIMING_STAT(MediaEncoder, OnBackBufferReady);
	...
	{
		if (GEngine && GEngine->GameViewport.Get())
		{
			FSceneViewport* SceneViewport = GEngine->GameViewport.Get()->GetGameViewport();
			if (!SceneViewport)
			{
				return;
			}
			FTexture2DRHIRef SceneBuffer = SceneViewport->GetRenderTargetTexture();
			if (!IsValidRef(SceneBuffer))
			{
				return;
			}
			ProcessVideoFrame(SceneBuffer);
		}	
	}	
}

void FMediaEncoder::FloatToPCM16(float const* floatSamples, int32 numSamples, TArray<int16>& out) const
{
...
void FMediaEncoder::ProcessAudioFrame(const float* AudioData, int32 NumSamples, int32 NumChannels, int32 SampleRate)
{
	...
	{
		UE_LOG(MediaEncoder, Warning, TEXT("Audio clock falling behind real time clock by %.3f seconds. Ajusting audio clock"), Now.GetTotalSeconds() - AudioClock);
...
void FMediaEncoder::ProcessVideoFrame(const FTexture2DRHIRef& FrameBuffer)
{
	...
	if(bDoFrameSkipping)
	{
		...
		UE_LOG(MediaEncoder, VeryVerbose, TEXT("time %.3f: captured %d, expected %d"), Now.GetTotalSeconds(), NumCapturedFrames + 1, NumExpectedFrames);
		if(NumCapturedFrames + 1 > NumExpectedFrames)
		{
			UE_LOG(MediaEncoder, Verbose, TEXT("Framerate control dropped captured frame"));
			return;
		}
	}
	...
	EncodeOptions.bForceKeyFrame = true;//jh
...
TSharedPtr<AVEncoder::FVideoEncoderInputFrame> FMediaEncoder::ObtainInputFrame()
{
...
void FMediaEncoder::SetVideoBitrate(uint32 Bitrate)
{
	NewVideoBitrate = Bitrate;
	bChangeBitrate = true;
}

void FMediaEncoder::SetVideoFramerate(uint32 Framerate)
{
	NewVideoFramerate = FMath::Clamp(Framerate, MinVideoFPS, MaxVideoFPS);
	bChangeFramerate = true;
}

bool FMediaEncoder::InitializeVideoConfig(uint32 Width, uint32 Height, uint32 Framerate, uint32 Bitrate, bool InIsAudio, bool InShowUI)
{
	VideoConfig.Codec = "h264";
	...
	else if (VideoConfig.Height == 1280)
	{
		VideoConfig.Width = 720;
		VideoConfig.Height = 1280;
	}
	else if (VideoConfig.Height == 1920)
	{
		VideoConfig.Width = 1080;
		VideoConfig.Height = 1920;
	}
	else if (VideoConfig.Height == 2560)
	{
		VideoConfig.Width = 1440;
		VideoConfig.Height = 2560;
	}
	else if (VideoConfig.Height == 3840)
	{
		VideoConfig.Width = 2160;
		VideoConfig.Height = 3840;
	}
	else
	{
		UE_LOG(MediaEncoder, Fatal, TEXT("MediaEncoder.ResY can only have a value of 720 or 1080 or 1440 or 2160 or 4320"));
		return false;
	}
...
void FMediaEncoder::UpdateVideoConfig()
{
...
void FMediaEncoder::OnEncodedAudioFrame(const AVEncoder::FMediaPacket& Packet)
{
...
void FMediaEncoder::OnEncodedVideoFrame(uint32 LayerIndex, const TSharedPtr<AVEncoder::FVideoEncoderInputFrame> InputFrame, const AVEncoder::FCodecPacket& Packet)
{
...
void FMediaEncoder::CopyTexture(const FTexture2DRHIRef& SourceTexture, FTexture2DRHIRef& DestinationTexture) const
{
